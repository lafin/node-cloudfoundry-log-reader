...
exports = module.exports = {
    recent: function (params, callback) {
        loginRequest(params, function (error, data) {
            request({
                method: 'GET',
                url: 'http' + (params.endpoints.ssl ? 's' : '') + '://' + params.endpoints.loggregator + '/recent?app=' + params.appsGuid,
                headers: {
    ...
    tail: function (params, callback) {
        loginRequest(params, function (error, data) {
            var ws = new WebSocket('ws' + (params.endpoints.ssl ? 's' : '') + '://' + params.endpoints.loggregator + ':4443/recent/?app=' + params.appsGuid, {
                headers: {
                    'Authorization': data.token_type + ' ' + data.access_token
                }
            });
